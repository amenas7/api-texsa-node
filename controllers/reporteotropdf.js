...
const getReporteByID = async (req, res) => {
    ...
    if ( resp_sol == 1 && resp_peso == 0 && resp_dolar == 0 ) {
        ...
        if (condicion == 'SI') {
            ...
            pdf.Output('F',`test.pdf`).toStream((error, stream) => {
                if (error) {
                    return res.end('Error creando el pPDF: '+err.stack)
                } else {
...
const reporte_soles_con_igv = async(req, res, id) =>{
    ...
    for (const producto of reg_detalle) {
        ...
        tabla += `<tr>
            ...
            <td style="vertical-align: middle"><img style="width: 80%" src="${producto.base}"></td>
